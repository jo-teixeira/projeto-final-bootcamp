...
class SessionServiceTest {
    ...
    @Test
    void loginOk(){
        Account account = new Account(null, "User", CountryEnum.ARGENTINA,
                "$2a$10$qFGi6LeysbHGxAwLL/wcMuCbjWPKqiVoSa0LVklVcjpCTzMjvZLry", null, null, null);
        when(accountRepository.findByUsername("User")).thenReturn(Optional.of(account));
        AccountResponseDTO accountResponseDTO = sessionService.login("User","contSD#&#!SD3");
        assertEquals("User", accountResponseDTO.getUsername());
    }
    ...
    void getAccountByIdTest(){
        Account accountToFind = new Account(null, "User", CountryEnum.ARGENTINA, "Pass",
                null, null, null);
